...
# ────────────────────────────────────────────────────────────────────────────
# Anthropic (Claude)  — ported from Session 7's gateway (llm_gatewayV7).
# ────────────────────────────────────────────────────────────────────────────
#
...
#   - there is no `response_format` field — to force JSON-schema output we
#     emulate it by registering a synthetic tool whose `input_schema` IS the
#     requested schema and forcing tool_choice to that tool. Anthropic then
#     guarantees the tool input matches the schema, which is the same
#     guarantee as native json_schema mode elsewhere. We lift the tool input
#     back into `text` so the gateway's existing structured-output validator
...
class AnthropicProvider(BaseProvider):
    ...
    def _translate_tools(self, tools):
        """Anthropic's tool shape is already very close to ours."""
        out = []
        for t in tools or []:
            d = t if isinstance(t, dict) else t.model_dump()
            out.append({
                "name": d["name"],
                "description": d.get("description", ""),
                "input_schema": _anthropic_clean_schema(
    ...
    def _translate_messages(self, messages):
        ...
        for m in messages:
            r = m.get("role")
            if r == "system":
                continue  # system handled at top level of the body
            if r == "tool":
                content = m.get("content")
    ...
    def _build_system(self, system_text, blocks, has_cache_marker, cache_system):
        if not system_text:
            return None
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        # response_format → emulate via a forced tool call whose input_schema
        # is the requested schema. See section header comment for rationale.
        structured_tool_name: Optional[str] = None
        local_tools = list(tools or [])
        local_tool_choice = tool_choice
        if response_format:
            rf = response_format if isinstance(response_format, dict) else response_format.model_dump(by_alias=True)
            schema = rf.get("schema")
            if schema:
                structured_tool_name = rf.get("name") or "emit_structured_output"
                local_tools.append({
                    "name": structured_tool_name,
                    "description": (
                        "Emit the final structured output. The `input` MUST "
                        "conform to the JSON schema declared in input_schema."
                    ),
                    "input_schema": schema,
                })
                local_tool_choice = {"type": "tool", "name": structured_tool_name}
        ...
        sys_payload = self._build_system(system_text, blocks, has_cache_marker, cache_system)
        if sys_payload is not None:
            body["system"] = sys_payload
        if local_tools:
            body["tools"] = self._translate_tools(local_tools)
            if local_tool_choice is not None:
                if isinstance(local_tool_choice, str):
                    if local_tool_choice == "auto":
                        body["tool_choice"] = {"type": "auto"}
                    elif local_tool_choice == "any":
                        body["tool_choice"] = {"type": "any"}
                    elif local_tool_choice == "none":
                        # Anthropic has no "none" mode — drop tools entirely.
                        body.pop("tools", None)
                else:
                    body["tool_choice"] = local_tool_choice

        reasoning_applied = False
        if reasoning and reasoning != "off":
            # Extended thinking. Requires Claude Opus/Sonnet 4+. Temperature
            # MUST be 1 when thinking is enabled — Anthropic 400s otherwise.
            budget = {"low": 1024, "medium": 4096, "high": 16384}.get(reasoning, 0)
            if budget:
        ...
        async with httpx.AsyncClient(timeout=180) as c:
            ...
            if r.status_code != 200:
                # One retry stripping thinking on 400 (model may not support it).
                ...
                if r.status_code != 200:
                    raise ProviderError(
                        f"anthropic HTTP {r.status_code}: {r.text[:400]}",
                        status=r.status_code,
                        retryable=(r.status_code not in (400, 401)),
                    )
            d = r.json()
            ...
            # Structured-output emulation: lift the forced tool's input back
            # into `text` (as JSON) and remove it from tool_calls so the
            # caller sees a plain JSON reply, not a tool call.
            if structured_tool_name:
                matched = [tc for tc in tool_calls_out if tc["name"] == structured_tool_name]
                if matched:
                    text = json.dumps(matched[0]["arguments"])
                    tool_calls_out = [tc for tc in tool_calls_out if tc["name"] != structured_tool_name]
...
def _anthropic_clean_schema(schema: dict) -> dict:
    """Inline `$ref`s (Pydantic emits them for nested models) and strip the
    JSON-Schema meta keys Anthropic doesn't accept on tool input_schema.
    Reuses the Gemini inliner — the ref shape is identical."""
    if not isinstance(schema, dict):
        return schema
    schema = _gemini_inline_refs(schema)
    drop = {"$schema", "$defs", "definitions"}
    def strip(node):
